\begin{titlepage}
\begin{center}
\noindent {\large \textbf{UNIVERSITÉ DE LYON - LUMIÈRE LYON 2}} \\
\vspace*{0.3cm}
\noindent {\LARGE \textbf{ECOLE DOCTORALE INFOMATHS}} \\
%\noindent \textbf{SCIENCES ET TECHNOLOGIES DE L'INFORMATION \\ ET DE LA COMMUNICATION} \\
\vspace*{0.5cm}
\noindent \Huge \textbf{P H D\ \ T H E S I S} \\
\vspace*{0.3cm}
\noindent \large {to obtain the title of} \\
\vspace*{0.3cm}
\noindent \LARGE \textbf{PhD of Computer Science} \\
\vspace*{0.3cm}
\noindent \Large of the University of Lyon - Lumière 2\\
%\noindent \Large \textbf{Specialty : \textsc{Computer Science}}\\
\vspace*{0.4cm}
\noindent \large {Defended by\\}
\noindent \LARGE Edmundo-Pavel \textsc{Soriano-Morales} \\
\vspace*{0.8cm}
\noindent {\Huge \textbf{Hypergraphs and information fusion for term representation enrichment. Applications to word sense disambiguation and named entity recognition}} \\
\vspace*{0.8cm}
\noindent \Large Thesis Advisor: Sabine \textsc{Loudcher} and Julien \textsc{Ah-Pine} \\

\vspace*{0.2cm}
\noindent \Large prepared at Université de Lyon 2, \textsc{ERIC} Laboratory
\vspace*{0.2cm}
\noindent \large defended on  \\
\vspace*{0.1cm}
\end{center}
\noindent \large \textbf{Jury :} \\
\begin{center}
\noindent \large 
%\begin{tabular}{llcl}
%      \textit{Reviewers :}	& Patrick \textsc{Clarysse}		& - & CNRS (CREATIS)\\
%				& Louis \textsc{Collins}		& - & McGill University\\
%      \textit{Advisor :}	& Grégoire \textsc{Malandain}		& - & INRIA (Asclepios)\\
%      \textit{President :}	& Nicholas \textsc{Ayache}		& - & INRIA (Asclepios)\\
%      \textit{Examinators :}   & Pierre-Yves \textsc{Bondiau}          & - & Centre Antoine Lacassagne (Nice)\\
%      				& Guido \textsc{Gerig}			& - & University of North Carolina\\
%      				& Vincent \textsc{Grégoire}		& - & Université Catholique de Louvain\\
%      \textit{Invited :}		& Hanna \textsc{Kafrouni}		& - & DOSISoft S.A.
%\end{tabular}
\end{center}
\end{titlepage}
\sloppy

\titlepage
