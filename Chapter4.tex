...
In this subsection we present an analysis on the results obtained with the combination fusion operators shown in above. Namely, we set to understand how each addition of fusion operators helps to improve the result of the task at hand. We focus on the most successful fusion combination found for the three tested corpora. For simplicity, we focus on the Wikigold corpus. Thismost performing fusion combination is indeed shown in Table \ref{tab:ner-nf1} and it is expressed as:    
\begin{equation}\label{eq:big_fusion}
E_{\alpha=0.95}(\mlex,  E(E(\mstd, 	 L(\mstd, X(\ssyn, \mstd))), L(\mlex, X(\ssyn, \mlex))))
\end{equation}

This fusion is principally base on early fusion operators. While we defined them in Chapter \ref{chap:ling_net} as a binary function, we can also write it as a $n$-ary function which concatenates all the input values\footnote{The parameter $\alpha$ applies exclusively between the first input and the rest of them.}. Thus, equation \ref{eq:big_fusion} can be written as:
\begin{equation}\label{eq:big_fusion2}
%wE_{\alpha=0.95}(\underbrace{\mlex}_{\circled{1}},\overbrace{\mstd}^{\circled{2}},\underbrace{L(\mstd, X(\ssyn, \mstd))}_{\circled{3}}, \overbrace{L(\mlex, X(\ssyn, \mlex))}^{\circled{4}})
...
As can be seen, the operation is a concatenation of four elements, the feature matrices $\mlex$ and $\mstd$, and two late fusions, each containing a cross early fusion. The analysis we make tries to elucidate the role of each numbered fusion combination. To this end, we analyze these elements cumulatively, i.e., we begin studying element \circled{1}, that is, the matrix $\mlex$ individually, then the early fusion between \circled{1} and \circled{2}, and eventually parts \circled{3} and \circled{4}. Explicitly, these numbered operations are:
\begin{enumerate}
\item[\circled{1}] $\mlex$
\item[\circled{2}] $E_{\alpha=0.95}(\mlex, \mstd)$
\item[\circled{3}] $E_{\alpha=0.95}(\mlex, \mstd, L(\mstd, X(\ssyn, \mstd)))$
\item[\circled{4}] $E_{\alpha=0.95}(\mlex, \mstd, L(\mstd, X(\ssyn, \mstd)), L(\mlex, X(\ssyn, \mlex)))$
\end{enumerate}
\paragraph{Per Entity Performance}
First, we are interested into discovering what is the contribution, in terms of improving the recognition of a specific type of named entity, of each of the operations included in the complete fusion operation of Equation \ref{eq:big_fusion2}. The Table 
...
\toprule
  & ALL           & LOC                   & MISC         & ORG                   & PER                    \\ \midrule
1 & 38.03         & 49.02                 & 30.24        & 27.49                 & 41.52                  \\
2 & 55.53 (17.50) & 65.04 (16.02)         & 40.03 (9.79) & 39.46 (11.97)         & \textbf{69.19 (27.67)} \\
3 & 56.11 (0.58)  & 65.75 (0.71)          & 40.26 (0.23) & \textbf{41.13 (1.67)} & 68.99 (-0.20)          \\
4 & 56.28 (0.17)  & \textbf{66.08 (0.33)} & 40.49 (0.23) & 41.07 (-0.06)         & 69.31 (0.32)           \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{figure}
	...
	\begin{subfigure}[t]{.5\textwidth}
	\centering
	\includegraphics[width=0.9\linewidth]{images/Chapitre4/M2_1140.pdf}
	\caption{Fusion of decisions.}
	\label{fig:lf1}
	\end{subfigure}% 
	\begin{subfigure}[t]{.5\textwidth}
	\centering
	\includegraphics[width=0.9\linewidth]{images/Chapitre4/M3_1140.pdf}
	\caption{Fusion of similarity matrices.}
	\label{fig:lf2}	
	\end{subfigure}
	\caption{Late fusion possibilities. To the left, fusion of knowledge discovery models' decisions. To the right, fusion of similarity matrices.}
...
\begin{figure}
	...
	\begin{subfigure}[t]{.5\textwidth}
	\centering
	\includegraphics[width=0.9\linewidth]{images/Chapitre4/M3_31756.pdf}
	\caption{Fusion of decisions.}
	\label{fig:lf1}
	\end{subfigure}% 
	\begin{subfigure}[t]{.5\textwidth}
	\centering
	\includegraphics[width=0.9\linewidth]{images/Chapitre4/M4_31756.pdf}
	\caption{Fusion of similarity matrices.}
	\label{fig:lf2}	
	\end{subfigure}
	\caption{Late fusion possibilities. To the left, fusion of knowledge discovery models' decisions. To the right, fusion of similarity matrices.}
