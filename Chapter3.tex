...
\paragraph{NER supervised prediction}

In order to determine the correct set of tags that identify the words within a phrase, NER is usually solved by using supervised structured algorithms. In particular, we focus on the  structured perceptron by \cite{Collins2002} sequence labeling algorithm. The general intuition of this method is introduced below. We chose this method as it is relatively simple and performs as well as more complex probabilistic approaches (e.g., conditional random fields, hidden markov models)  \cite{Daume2006,daume2012course}. 




Like  the well-known perceptron algorithm, the structured perceptron is an online method that learns to classify one example at a time. The goal of this algorithm is to determine a label for each structured input. Formally, for an input $x \in \mathbb{X}$, we want to predict a class $\hat{y} \in \mathbb{Y}$ that maximizes the following product: 

\begin{equation}
\hat{y}= \argmax_{\hat{y}\in \mathbb{Y}}w\cdot \Phi(x,\hat{y})
\end{equation}
where $\Phi(x,\hat{y})$ is the vector of representing  the feature space of input  $x$ with respect to each of the  possible labels $\mathbb{Y}=\{1,\dots,\textit{K}\}$. Indeed,  $\Phi(x,\hat{y})$ indicates a certain level of compatibility between input $x$ and each label $y$. 
...
The pseudocode for the structured perceptron is shown in Algorithm \ref{alg:struct_perc}.  For each input, we obtain its predicted class and determine whether it is qual to the true label. If it is not the case, we update the weight vector.

\begin{algorithm}[!htp]
\SetAlgoLined
\KwIn{Examples $x \in \mathbb{X}$}
\KwIn{True labels  $y \in \mathbb{Y}$ }
\KwIn{Max number of iterations $MaxIteration$}
\KwOut{A vector of lerned weights $w$}
\For{$Iteration=1\dots MaxIterations$}
{
	\ForEach{$(x,y) \in \mathbb{X},\mathbb{Y}$}
	{
		$\hat{y}= \argmax_{\hat{y}\in \mathbb{Y}}w\cdot \Phi(x,\hat{y})$
...
The main issue with this algorithm is the computation of $\argmax_{\hat{y}\in \mathbb{Y}}w\cdot \Phi(x,\hat{y})$ in line 3 of Algorithm \ref{alg:struct_perc}. As said before, we are looking for the sequence of labels $\hat{y}$  that maximize this product. This implies a very large search over all the possibilities that grows exponentially with the number of possible tags $\mathbb{Y}$. If we consider a phrase with $L$ words, and a tagset of $k$ labels, the total number of possibilities to explore would be $L^k$. For example, for a phrase with 10 words, and 5 labels, we would need to explore a space of $10^5=100,000$ possibilities. This exploration then tends to be computationally unfeasible for a large number of phrases and/or larger number of words per phrase.
...
Another popoular clustering approach to automatically induce word senses is spectral clustering. This approach is generally interesting as the data does not necesseraly need the input data to be expressed as a graph structure. The algorithm consists in projecting the
